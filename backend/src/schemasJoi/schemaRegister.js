...
const schemaRegister = {
  validationRegister: async (body) => {
    const schema = Joi.object({
        ...
        .pattern(/^(?=.*\d)(?=.*[}{,.^?@~=+\-_\/*\-+.\|])(?=.*[a-z])(?=.*[A-Z])[0-9a-zA-Z$*&@#]{8,}$/)
        .messages({
          "string.pattern.base": `${httpstatuscode.BAD_REQUEST}|O "password" precise ter 8 digitos, incluindo pelo menos uma letra maiusculas, menusculas, um numero e um caracter especial`,
